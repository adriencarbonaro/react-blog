...
class P1 extends Component {
	render () {
		return (
			<>
Arrivée très tardive le mardi 30 octobre au soir dans une auberge de jeunesse branchée, le “St Christopher’s Inns Berlin”, dans le quartier Mitte.
...
class P3 extends Component {
	render () {
		return (
			<>
Après cette première matinée remplie, direction Markethalle Neun pour la pause déjeuner. Il s’agit d’un grand hangar où se trouvent différents stands de street food et où l’on peut manger sur des longues tables au centre, étonnamment pas très touristique. Nous avons choisi de goûter les burger de chez Kumpel & Keule qui sont super bons et en dessert nous avons craqué pour les crêpes d’un stand tenu par un français ! 
Puis l’après-midi nous avons à nouveau sillonné Berlin à vélo pour voir les incontournables de la ville : Checkpoint Charlie, le mémorial des juifs, la porte de Brandebourg ainsi que le Reichstag. En ayant réservé avant, nous avons pu monter dans la coupole au sommet pour admirer le coucher de soleil sur la ville et notamment sur la porte qui se trouve juste en face, un magnifique spectacle ! 
Fatigués d’avoir pédalé toute la journée nous avons loué un scooter pour nous rendre au restaurant : le Prater Biergarten, une brasserie familiale typiquement allemande. Je ne me souviens que de la bière et du strudel aux pommes en dessert … c’était donc une super soirée
			</>
		)
	}
}
...
class P4 extends Component {
	render () {
		return (
			<>
Nouveau petit déjeuner à l’auberge puis c’est reparti pour une journée à vélo. Nous avons pédalé jusqu’au musée “Topographie de la terreur”. Ce musée retrace l’histoire de l'Allemagne au XXe siècle et notamment la montée du nazisme. Pour le déjeuner, après avoir tenté notre chance dans plusieurs restaurants repérés sur internet qui se sont tous avérés fermés. Nous avons atterri par hasard dans un petit restaurant mexicain le “Santa Maria” qui n’a absolument rien à envier aux autres, c’était super bon ! Il se trouve juste à côté du Mauerpark où nous nous sommes promenés avant de reprendre les vélos pour nous rendre jusqu’à l’East Side Gallery. Nous avons ensuite marché le long des vestiges du mur en admirant les différentes fresques. Le mur étant clairement sans fin, nous avons atterri dans le café du “Shulz Hotel Berlin Wall” pour nous réchauffer et prendre un chocolat chaud le temps de souffler un peu avant de reprendre la route. 
...
Nous sommes donc arrivés pile pour l’heure de l’apéro au Markethalle Neun où nous avons pris un verre de vin. Nouveau tour en scooter et traversée de Berlin de nuit, tout était illuminé c’était magnifique et nous avons été diner dans un restaurant italien “Vino” proche de l’auberge de jeunesse.
			</>
		)
	}
}
...
class P5 extends Component {
	render () {
		return (
			<>
Dernier jour à Berlin. Nous avons cette fois-ci passé la matinée sur l'île des musées qui se situe au centre de Berlin. Nous sommes montés en haut du Berliner Schloss afin d’avoir une belle vue sur l’île, l’esplanade au centre et les musées en face. Puis nous nous sommes promenés sur l’esplanade. Une ambiance très agréable, des gens qui bouquinent sur la pelouse, des musiciens qui jouent … comme un air de printemps au mois de novembre. Nous avons ensuite marché dans les rues et avons déjeuné chez “Keizer Soze”. Puis balade et shopping dans les rues aux alentours avant de retourner à l’auberge de jeunesse pour récupérer nos affaires et direction l’aéroport. 
